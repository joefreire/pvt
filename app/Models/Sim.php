...
use Carbon\Carbon;
class Sim extends Model implements Auditable
{
	...
	{
		return $this->HasMany(LinkagemSim::class, 'idUploadSIM', 'id');
	}	
	...
	{
		if($this->DTNASC == "99/99/9999" || $this->DTOBITO == "99/99/9999"){
			return 'Sem Informação';
		}
		if(!empty($this->DTNASC) && !empty($this->DTOBITO)){
			try {
				$DTNASC = \Carbon\Carbon::createFromFormat('d/m/Y',$this->DTNASC);
				$DTOBITO = \Carbon\Carbon::createFromFormat('d/m/Y',$this->DTOBITO);
				return $DTNASC->diffInYears($DTOBITO);
	...
	public function getAcidenteTransitoAttribute($value)
	{
		$CAUSABAS = trim($this->getOriginal('CAUSABAS'));
		$CAUSABAS_O = trim($this->getOriginal('CAUSABAS_O'));
		if((!empty($CAUSABAS) && $CAUSABAS[0] == 'V') || (!empty($CAUSABAS_O) && $CAUSABAS_O[0] == 'V')){
			return 'Sim';
		}else{
			return 'Não';
		}
