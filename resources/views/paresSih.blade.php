...
<div class="col-md-12">    
	...
	<div id="_SIH" style="display:none;">
		<span>Primeiro você deve fazer o upload do  <a href="{{route('sih')}}" class="linkSih">SIH</a></span>
	</div>
	<div id="Linkagem" style="display:none;">
		<div class="panel panel-default">
			<div class="panel-heading clearfix"> 
				<span id="titulo-painel" class="h3-semlinha"> Possíveis pares ainda não marcados na Linkagem entre o SIH e a Lista Única</span>
			</div>
					...
					<table class="table table-bordered table-hover display nowrap dataTable" id="table" width="100%">
						<thead>
							<tr>
								...
								<th>Data Nascimento <BR> SIH</th>
								<th>Data Acidente <BR> Lista Única</th>
								<th>Data Internação <BR> SIH</th>
								<th>Sexo<BR>Lista Única</th>
								<th>Sexo<BR>SIH</th>
								<th>DIAGNOSTICO<BR>SIH</th>
								<th>NOME DA MÃE<BR>Lista Única</th>
								<th>NOME DA MÃE <BR>SIH</th>
	...
	<div class="modal-dialog">
		<div class="modal-content">
			<div class="modal-header">
				...
				<h4 class="modal-title" id="informacoesLabel">Pares Verdadeiros Lista Única / SIH</h4>
			</div>
			<form method="POST" action="{{ route('sih.salvaPares') }}">
				@csrf
...
<script type="text/javascript">
	...
	$(document).ready(function() {
		function format ( d ) {
			...
			var tableSih = '<h5> Dados Originais SIH </h5><table class="table table-bordered" width="100%"><thead><tr>';

			$.each(d.dados_sih, function(key, value) {
				tableSih = tableSih + '<th>'+key+'</th>';
			});
			tableSih = tableSih + '</tr></thead><tbody><tr>';
			$.each(d.dados_sih, function(key, value) {
				tableSih = tableSih + '<td>'+String(value)+'</td>';
			});
			tableSih = tableSih + '</tr></tbody><table>';
			
			return tableQuadro+tableVitima+tableSih;
		}
		var table = $('#table').DataTable({
			...
			buttons: [ 
			// { extend: 'selectAll', text: 'Selecionar Todos' },
		...
		$('#ParFalso').click(function () {
			...
			$.each(data, function(key, value) {
				$('#pares').append('Nome Lista Única: ' + value.vitima.NomeCompleto + '<BR> Nome SIH :' + value.sih.NOME + '<BR>');
				$('#ids_pares').append('<input id="ids' + value.id + '" name="ids_pares[]" type="hidden" value="' + value.id + '">');
				$('#informacoesLabel').html('Pares Falsos Lista Única / SIH');
		...
		$('#ParVerdadeiro').click(function () {
			...
			$.each(data, function(key, value) {
				...
				if ((value.vitima.DataAcidente == value.sih.DT_INTERNA) && (value.vitima.NomeCompleto == value.sih.NOME) && (value.vitima.Sexo == value.SexoSih) && (value.vitima.DataNascimento == value.sih.DT_NASC)) {
					$('#pares').append('<div class="alert alert-success"> \n\
						...
						Nome Lista Única: ' + value.vitima.NomeCompleto + '<br> Nome SIH :' + value.sih.NOME + '\n\
						<BR>Data Nascimento Lista Única: ' + value.vitima.DataNascimento + ' <BR> Data Nascimento SIH :' + value.sih.DT_NASC + '\n\
						<BR>Sexo Lista Única: ' + value.vitima.Sexo + ' <BR> Sexo SIH :' + value.SexoSih + '\n\
						<BR>Data Acidente Lista Única: ' + value.vitima.DataAcidente + ' <BR> Data Óbito :' + value.sih.DT_INTERNA + '\n\
						<BR>Mãe Lista Única: ' + value.vitima.NomeMae + ' <BR> Mãe SIH :' + value.sih.NOME_MAE + '\n\
						<BR>DIAGNOSTICO: ' + value.sih.DIAG_PRI + '\n\
						</div>');
				} else {
					var data_lista = new Date(value.vitima.DataAcidente.replace(/(\d{2})[-/](\d{2})[-/](\d{4})/, "$2/$1/$3"));
					var data_obito = new Date(value.sih.DT_INTERNA.replace(/(\d{2})[-/](\d{2})[-/](\d{4})/, "$2/$1/$3"));
					...
					if (data_obito > trinta_dias) {
						$('#pares').append('<div class="alert alert-danger"> \n\
							...
							<BR>Data Acidente Lista Única: ' + value.vitima.DataAcidente + ' <BR> Data Óbito :' + value.sih.DT_INTERNA + '</STRONG>\n\
							<BR>DIAGNOSTICO: ' + value.sih.DIAG_PRI + '<BR>IDENTIFICADOR ACIDENTE: ' + value.quadro_multiplo.IdentificadorAcidente + '\n\
							</div>');
					} else {
						if (data_lista <= data_obito) {
							$('#pares').append('<div class="alert alert-info"> \n\
								<button type="button" onclick="removeID(' + value.id + ');" class="close" data-dismiss="alert" aria-label="Close"> <span aria-hidden="true">&times;</span></button> \n\
								<strong>ATENÇÃO: VERIFIQUE TODOS OS DADOS ANTES DE APROVAR</STRONG><BR><BR>\n\
								' + ((value.vitima.NomeCompleto != value.sih.NOME) ? '<STRONG>' : '') + '\n\
								Nome Lista Única: ' + value.vitima.NomeCompleto + '<br> Nome SIH :' + value.sih.NOME + '\n\  \n\
								' + ((value.vitima.NomeCompleto != value.sih.NOME) ? '</STRONG>' : '') + '\n\
								' + ((value.vitima.DataNascimento != value.sih.DT_NASC) ? '<STRONG>' : '') + '\n\
								<BR>Data Nascimento Lista Única: ' + value.vitima.DataNascimento + ' <BR> Data Nascimento SIH :' + value.sih.DT_NASC + '\n\
								' + ((value.vitima.DataNascimento != value.sih.DT_NASC) ? '</STRONG>' : '') + '\n\
								' + ((value.vitima.Sexo != value.SexoSih) ? '<STRONG>' : '') + '\n\
								<BR>Sexo Lista Única: ' + value.vitima.Sexo + ' <BR> Sexo SIH :' + value.SexoSih + '\n\
								' + ((value.vitima.Sexo != value.SexoSih) ? '</STRONG>' : '') + '\n\
								' + ((value.vitima.DataAcidente != value.sih.DT_INTERNA) ? '<STRONG>' : '') + '\n\
								<BR>Data Acidente Lista Única: ' + value.vitima.DataAcidente + ' <BR> Data Óbito :' + value.sih.DT_INTERNA + '\n\
								' + ((value.vitima.DataAcidente != value.sih.DT_INTERNA) ? '</STRONG>' : '') + '\n\
								' + ((value.vitima.NomeMae != value.sih.NOME_MAE) ? '<STRONG>' : '') + '\n\
								<BR>Mãe Lista Única: ' + value.vitima.NomeMae + ' <BR> Mãe SIH :' + value.sih.NOME_MAE + '\n\
								' + ((value.vitima.NomeMae != value.sih.NOME_MAE) ? '</STRONG>' : '') + '\n\
								<BR>DIAGNOSTICO: ' + value.sih.DIAG_PRI + '<BR>IDENTIFICADOR ACIDENTE: ' + value.quadro_multiplo.IdentificadorAcidente + '\n\
								</div>');
...
$('#informacoesLabel').html('Pares Verdadeiros Lista Única / SIH');
$('#informacoes').modal('show');
			...
			if ($('#Ano').val() !== '' && $('#Trimestre').val() !== '' && $('#CodCidade').val() !== '' ){
				...
				$.ajax(
				{
					...
					success: function (data, textStatus, jqXHR)
					{
						...
						$('#QtdSIH').html('Nesse período temos '+data.sih+' vítimas no SIH');
						...
						if (data.processo > 0 ){
							...
							$('#Linkagem').show();
							$('#Pendencia').hide();
						}else if (data.lista > 0 && data.sih == 0) {
							$('#desabilitar').hide();
							$('#_delete').hide();
							$('#_processo').hide();
							$('#_ListaUnica').hide();
